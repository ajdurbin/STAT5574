...
Data, source code, and output were all separated for each project. The files `Environment.R` and `Master Control.R` are sourced through a `Bash` script and they appropraitely load all other functions. `Environment.R` sets the file paths for input, output, functions, and markdown scripts. It also provides discriptions of the analysis to be included in the markdown script later. Most importantly, it creates the nodes used for parallel computation using the `parallel` package in `R`.
