...
Common estimating procedures are M and S estimation. M estimation, similar to OLS, minimizes a function of the residuals. S estimation minimizes the dispersion of the residuals. MM estimation is a combination of M and S estimation, it has the combination of high breakdown point, bounded influence function, and is slightly less asymptotically efficient than OLS. Two MM estimation procedures are Hubert and Tukey's Biweight. Their weighting functions are compared in the Figure 2. 

```{r, echo = FALSE, fig.align='center', fig.cap='Weighting functions for Huber and  Tukey estimation procedures.', fig.width=4, fig.height=4}

# knitr::include_graphics("Weight.png")

```

Note that Tukey's Biweight only gives full weight to observations with zero residual, that is observations directly on the regression line. It then down-weights observations more severely than the Hubert criteria. Note that Hubert estimation gives observations *near* the regression line full weight and only gives zero weight asymptotically. Figure 3 provides an example of robust vs OLS regression fits.

```{r, echo = FALSE, fig.align='center', fig.cap='Robust versus ordinary least squares regression lines with two iterations.', fig.width=3, fig.height=3}

# knitr::include_graphics("rlm_ols.png")
...
\item Calculate $w_i$ using $u_i$ according to Tukey or Hubert
\item Calculate $\hat{\beta}^b$ suing WLS with $w_i$
