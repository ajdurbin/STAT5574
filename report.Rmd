...
I also became very proefficient with several `R` packages and functions. Because programs were run remotely and not through a GUI, we made extensive use of log files for scripts. Combined with the CodeCommit version control, we could easily compare the current run to the previous run to make sure there was no user error when configuring options.
